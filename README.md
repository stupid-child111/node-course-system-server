...
### 实现软删除和多选操作
- 在Sequelize里，**实现软删除**是非常容易的事情。数据库中要增加deletedAt字段，来记录当前文章是否被删除。模型里，要添加paranoid: true。- 现在再删除文章， 就会变成软删除了。原理就是在deletedAt字段添加了个时间而已。
- 在查的时候，我们**只查deletedAt字段为空的记录。用户就以为数据被删掉了**，其实并没有。
- 要查询被软删除的记录，需要添加**paranoid: false**。
- 从**回收站恢复数据，可以用restore方法**。
- 要彻底删掉记录，可以在删除时，传递**force: true**参数。
- 还有一个值得思考的地方是，很多项目都有删除用户的功能。但是用户往往关联了大量的数据，特别是涉及到了支付相关的项目。如果真的把用户删掉了，那么将来追踪订单都会变的困难。这种情况下，考虑使用软删除，也是一种解决办法。

### 使用redis缓存
- Redis是一个开源的、**基于内存的数据存储系统**，可以用作数据库、缓存和消息中间件。**Redis的数据都存储在内存中**，这样它的读写速度就非常快。而且还会**将内存中的数据写入本地硬盘，可以在服务器重启后，自动将数据恢复到内存中**。
- connect --> set(key,value) --> get(key) --> disconnect
- 对系统设置接口进行redis缓存
  - 第一次使用SQL查询到数据库之后,设置key值,将其缓存到数据库,下次直接查询key值,取出数据
  - 如果修改系统设置信息,就delKey(key),防止读取时取出redis缓存

#### Redis缓存优化策略
- 前台缓存一切,后台可以视情况而定
- 分类 首页 
  - 需要查询之后设置缓存  -->  修改之后清除缓存(自定义setKey getKey delKey方法) 
- 文章列表 
  - 对于有分页的接口的缓存设置
    - key的命名，中间用的是:号，而不是空格，或者其他横线。**articles:currentPage:pageSize**
      - 第一、这是非常规范的做法，这是Redis社区的约定俗成的命名方式。
      - 第二、这种:的写法，Redis可以**支持在articles:后面跟随*号通配符**，批量找到对应的数据。
      - 第三、Redis客户端中可以对这种格式的数据，进行分组管理。
  - 更新数据后  --> 先通过自定义方法**getKeysByPattern** 获取缓存的数据 --> 再删除缓存
- 文章详情
  - 设置 key(article:${id})<命名规范>

#### 后续接口缓存优化实现以及清除所有缓存接口实现
- 1.设置缓存
  - 1.1.前台：课程列表
  - 1.2.前台：课程详情
  - 1.3.前台：章节详情
...
### 验证码接口实现
- 生成验证码可以使用**svg-captcha**。
- **验证码保存在Redis**中，key的名字可以通过uuid生成唯一标识。
- 生成验证码的接口，要**返回captchaKey和captchaData**。
- captchaKey是Redis中key的名字。captchaData是生成的svg图片。
- 当用户提交验证码时，需要将接口返回的captchaKey，和用户填写的验证码内容，一起提交过来。
- **由于在很多地方都可能用到验证码，可以将检查验证码是否正确的代码，放在自定义中间件(validate-captcha)中。当哪里要用验证码，就在哪里调用一下**。
- 中间件里，通过captchaKey在Redis中，找到对应的值，再做比对。
- **验证码使用完成后，记得要将它从Redis中删掉**，防止被人重复使用。
...
### 使用消息队列的优点
目的:实现邮件异步发送
直接将发送邮件接口的await删除(但是不等待发送邮件异步执行的结果,会导致catch无法捕捉错误,而且失败后整个程序崩溃,其他接口同样无法使用,在定义的发送邮件方法(sendMail)内部进行捕获)
- 1.解耦,将发送邮件功能独立出去,不会影响到主应用的性能和可用性
- 2.增加并发,对于高并发的情况,将任务放入队列中可以起到缓冲作用,增加程序的吞吐量,防止后端服务因为瞬间请求过多而崩溃
- 3.错误处理,如果消息对列中的任务处理失败,可以进行重试,确保任务成功完成

#### 使用场景
- 发送电子邮件、发送短信、应用内通知、文件处理、数据分析与报告生成、订单处理、秒杀


### 大会员模式
#### 一、收费模式选择
- 项目采用会员制收费（第二种模式）：用户开通会员后，在有效期内可浏览全站所有收费课程；相比按课程单独收费，该模式更便于演示支付流程，且符合项目现有会员系统基础。
#### 二、课程与章节的收费设置
为区分免费 / 收费内容，对课程表（Courses）和章节表（Chapters）进行改造：
- 1.**添加free字段：**
通过 Sequelize 迁移为两张表新增free字段（布尔类型，true为免费，false为收费），并为 Courses 表的free字段添加索引（优化查询性能）。
课程表的free字段用于展示（告知用户课程是否含收费章节），章节表的free字段用于实际权限判断（核心）。
- 2.**模型与路由适配：**
在模型（Course、Chapter）中添加free字段的定义与验证，确保数据合法性。
在后台路由的增删改接口中，将free字段纳入参数白名单，支持通过后台管理系统设置课程 / 章节是否收费。
#### 三、会员套餐管理
- 设计会员套餐体系，用于用户购买会员，具体包括：
- Membership 表设计：
  - 存储会员套餐信息，字段包括名称（name）、时长（durationMonths，单位：月）、价格（price）、排序（rank）、描述（description）。
  - 通过迁移创建表结构，添加数据验证（如价格、时长必须为正数），并生成种子数据（如月付、年付套餐）。
- 前后台路由实现：
- 后台路由：支持会员套餐的增删改查（用于管理员维护套餐）。
- 前台路由：查询会员套餐列表（用于用户选择购买），并添加缓存优化查询性能。
#### 四、用户会员信息管理
为记录用户的会员身份及有效期，对用户表（Users）改造：
添加membershipExpiredAt字段：
通过迁移新增该字段（日期类型），记录会员有效期，并添加索引（优化过期用户查询）。
模型中添加字段定义（无需人工验证，由程序自动设置）。
用户角色适配：
复用用户表现有role字段（0：普通用户，1：大会员，100：管理员），通过role区分用户身份。
生成测试用户（普通用户、有效期 1 年的会员、管理员），用于权限测试。
#### 五、会员权限判断逻辑
控制用户对收费章节的访问权限，核心逻辑：
登录校验：为章节接口添加登录中间件，强制用户登录后才能访问（无论章节是否免费，提高注册量）。
权限判断：
若章节为免费（free: true），直接允许访问。
若章节为收费（free: false），验证用户role：仅大会员（role: 1）或管理员（role: 100）可访问，否则返回 403 错误（提示购买会员）。
#### 六、核心总结
收费模式：选择会员制，便于统一管理全站收费内容。
数据设计：通过free字段标记课程 / 章节是否收费，membershipExpiredAt记录会员有效期，role区分用户身份。
权限控制：登录后才能访问章节，收费章节仅对会员 / 管理员开放。
可扩展思路：可结合按课程收费模式（如会员享折扣），或调整免费章节的访问策略（如未登录用户仅显示部分内容）。